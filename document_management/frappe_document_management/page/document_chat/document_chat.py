...
from document_management.frappe_document_management.rag.evaluation import get_latest_report
from document_management.frappe_document_management.rag.index import index_status, rebuild_index
...
def ask_question(session, query):
    ...
    job = frappe.enqueue(
        "document_management.frappe_document_management.rag.service.generate_answer",
        queue="long",
...
def rebuild_rag_index():
    ...
    job = frappe.enqueue(
        "document_management.frappe_document_management.rag.index.rebuild_index",
        queue="long",
        timeout=3600,
        enqueue_after_commit=True,
    )
...
def run_rag_evaluation(include_generation=0):
    ...
    job = frappe.enqueue(
        "document_management.frappe_document_management.rag.evaluation.run_evaluation",
        queue="long",
