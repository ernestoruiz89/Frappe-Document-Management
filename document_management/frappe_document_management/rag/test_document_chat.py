...
from document_management.frappe_document_management.page.document_chat.document_chat import (
    _get_active_message,
    cancel_message,
    get_message,
)
...
from document_management.frappe_document_management.utils.ocr_worker import _aggregate_pages
from document_management.frappe_document_management.rag.service import (
    SYSTEM_PROMPT,
...
from document_management.frappe_document_management.doctype.document.document import _sha256_file
...
from document_management.frappe_document_management.doctype.document_saved_view.document_saved_view import (
...
class TestDocumentChatSecurity(FrappeTestCase):
    ...
    def test_permanent_delete_requires_system_manager(self):
        with (
            patch(
                "document_management.frappe_document_management.page.document_management_console.document_management_console.frappe.get_roles",
                return_value=["All"],
            ),
    ...
    def test_document_moved_to_trash_is_not_still_allowed(self):
        ...
        with (
            patch(
                "document_management.frappe_document_management.rag.service.frappe.db.exists",
                return_value=True,
            ),
            patch(
                "document_management.frappe_document_management.rag.service.frappe.get_doc",
    ...
    def test_hybrid_results_are_deduplicated_and_capped_per_document(self):
        ...
        with (
            patch(
                "document_management.frappe_document_management.rag.index._semantic_search",
                return_value=semantic,
            ),
            patch(
                "document_management.frappe_document_management.rag.index._lexical_search",
                return_value=lexical,
            ),
            patch(
                "document_management.frappe_document_management.rag.index.rerank_texts",
                return_value=None,
            ),
            patch("document_management.frappe_document_management.rag.index._validate_meta"),
            patch(
                "document_management.frappe_document_management.rag.index._read_meta",
                return_value={},
            ),
            patch(
                "document_management.frappe_document_management.rag.index.embed_texts",
                return_value=[[1.0, 0.0]],
            ),
            patch(
                "document_management.frappe_document_management.rag.index._active_root",
                return_value=Path("generation"),
            ),
            patch(
                "document_management.frappe_document_management.rag.index.FileLock",
            ) as file_lock,
        ):
    ...
    def test_document_search_preserves_hybrid_rank_and_excerpt(self):
        ...
        with patch(
            "document_management.frappe_document_management.rag.index.search_chunks",
            return_value=ranked,
        ) as mocked_search:
    ...
    def test_semantic_search_discards_candidates_below_threshold(self):
        ...
        with (
            patch(
                "document_management.frappe_document_management.rag.index._load_faiss",
            ...
            patch(
                "document_management.frappe_document_management.rag.index.get_rag_config",
                return_value=config,
            ),
            patch(
                "document_management.frappe_document_management.rag.index._active_root",
            ...
            patch(
                "document_management.frappe_document_management.rag.index._load_chunk_metadata",
                return_value=[
                    {
                        "name": "chunk-1",
                        "document": "DOC-A",
    ...
    def test_persisted_ocr_pages_are_preferred_for_indexing(self):
        ...
        with (
            patch(
                "document_management.frappe_document_management.rag.index.frappe.db.exists",
                return_value=True,
            ),
            patch(
                "document_management.frappe_document_management.rag.index.frappe.get_all",
                return_value=rows,
            ),
        ):
    ...
    def test_stale_active_message_is_failed_and_released(self):
        ...
        with patch(
            "document_management.frappe_document_management.page.document_chat.document_chat.get_rag_config"
        ) as config:
