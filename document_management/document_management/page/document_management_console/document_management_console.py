...
def _enqueue_index_refresh(names, remove=False):
    job = frappe.enqueue(
        "document_management.document_management.rag.index.refresh_document_indexes",
...
def force_generate_pdf(doc_name):
    ...
    from document_management.document_management.doctype.document.document import convert_office_to_pdf
    try:
...
def reprocess_ocr(doc_name):
    ...
    frappe.enqueue(
        "document_management.document_management.utils.ocr_worker.process_ocr",
        doc_name=doc.name,
        queue="long",
        enqueue_after_commit=True,
    )
